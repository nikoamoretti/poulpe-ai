"""Adapter for launching Claude Code CLI sessions.

Mirrors ``codex_local.py`` but builds commands targeting
``app.runtime.claude_code_exec_worker`` and the ``claude`` binary.
"""
...
class ClaudeCodeAdapter(AgentAdapter):
    ...
    def send(self, session_id: str, message: str) -> None:
        # Claude Code -p mode doesn't support follow-up stdin messages
        if not self._simulation_modes.get(session_id, self.default_simulation_mode):
            raise ValidationError(
                "Real Claude Code sessions do not support live follow-up messages. "
    ...
    def _build_command(self, session_config: AgentSessionConfig) -> list[str]:
        ...
        if not session_config.command.strip():
            raise ValidationError("Real Claude Code execution requires a non-empty command.")
        command_parts = shlex.split(session_config.command)
        if not command_parts:
            raise ValidationError("Real Claude Code execution requires a valid command.")
        if not session_config.workspace_path:
            raise ValidationError("Real Claude Code execution requires a workspace path.")
        return [
            sys.executable,
            "-m",
            "app.runtime.claude_code_exec_worker",
            "--session-id",
            session_config.session_id,
            "--role",
            session_config.role.value,
...
ClaudeCodeLocalAdapter = ClaudeCodeAdapter
